...
PatchyRod::PatchyRod()
{
    ...
    if ( USE_DH )
    {
        DH_PREFACTOR_ = 500. / (double)SQR(N_PATCH_);
        MINUS_KAPPA_  = -2. * SIGMA_R;
        R_CUT_        = 5.  * SIGMA_R;
    ...
    else
    {
        ...
        // Bounding volume parameters
        BHierarchy->l_xh =  R_WCA_         / 2.;
        BHierarchy->l_yh =  R_WCA_         / 2.;
        BHierarchy->l_zh = (R_WCA_ + L_Z_) / 2.;
        
        BHierarchy->l_ch =  L_Z_           / 2.;
        BHierarchy->l_rc =  R_WCA_         / 2.;
    }
    
    R_INTEG = 2. * (BHull->l_ch + BHull->l_rc);
        ...
        for ( uint idx_center = 0; idx_center < N_PATCH_; ++idx_center )
        {
            ...
            for ( uint idx_theta = 0; idx_theta < N_RES_; ++idx_theta )
            {
                ...
                for ( uint idx_phi = 0; idx_phi < N_RES_; ++idx_phi )
                {
                    ...
                    Wireframe(0, idx) += -1./(2.*MINUS_KAPPA_) * sin(theta)*cos(phi);
                    Wireframe(1, idx) += -1./(2.*MINUS_KAPPA_) * sin(theta)*sin(phi);
                    Wireframe(2, idx) += -1./(2.*MINUS_KAPPA_) * cos(theta);
