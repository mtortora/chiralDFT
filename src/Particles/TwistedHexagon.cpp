// ===================================================================
/**
 * Twisted hexagon derived particle class.
 */
// ===================================================================
/*
 * TwistedHexagon.cpp: Version 2.1
 * Created 14/04/2018 by Maxime Tortora
 */
// ===================================================================
...
TwistedHexagon<number>::TwistedHexagon()
{
	...
	R_THRESHOLD_    = sqrt(SQR(R_HEX_/(N_R_-1.)) + SQR(L_Z_/(N_Z_-1.)))/2.;
	
	this->R_INTEG   = sqrt(SQR(2.*R_HEX_+2.*R_BCK_) + SQR(L_Z_)) + R_THRESHOLD_;
	this->V_INTEG   = CUB(2.*this->R_INTEG) * 16.*pow(PI, 6);
	
	this->V0        = SQR(R_HEX_)*L_Z_ * 3 * cos(PI/6.);
	this->V_EFF     = this->V0;
...
// ============================
template<typename number>
void TwistedHexagon<number>::Build(int mpi_rank)
{
	number r = R_HEX_ * cos(PI/6.);
	
	Matrix3X<number> Edge(3, N_R_);
	
	Matrix3X<number> Backbone (3, N_Z_);
	Matrix3X<number> Wireframe(3, 6*N_R_*N_Z_);
	Matrix3X<number> Face_r   (3, N_R_*N_Z_);
	
	Vector3<number>  Z_axis     = Vector3<number>::UnitZ();
	
	ArrayX<number>   R_grid     = ArrayX<number>::LinSpaced(N_R_, -R_HEX_/2., R_HEX_/2.);
	...
	for ( uint idx_f = 0; idx_f < 6; ++idx_f )
	{
		...
		Edge = Eigen::AngleAxis<number>(2.*PI/6., Z_axis).toRotationMatrix() * Edge;
	}
	...
	if ( USE_RAPID )
	{
		...
		this->Hull->l_xh = R_BCK_ + R_HEX_;
		this->Hull->l_yh = R_BCK_ + R_HEX_;
		this->Hull->l_zh = L_Z_ / 2.;
		
		this->Hull->l_ch = this->Hull->l_zh;
		this->Hull->l_cr = R_BCK_ + R_HEX_;
...
// ============================
template<typename number>
void TwistedHexagon<number>::Tesselate(const Matrix3X<number>& Wireframe, uint* num_tri)
{
	uint   ctr_tri(0);
...
// ============================
template<typename number>
void TwistedHexagon<number>::SaveWireframe(const Matrix3X<number>& Wireframe)
{
...
// ============================
template<typename number>
void TwistedHexagon<number>::SaveMesh(const Matrix3X<number>& Wireframe, uint num_tri)
{
	uint ctr_tri(0);
...
template class TwistedHexagon<float>;
template class TwistedHexagon<double>;
