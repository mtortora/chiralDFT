...
void SimManager::InitRun(int mode)
{
    ...
    else
    {
        ...
        else if ( mode == ODF_LEGENDRE ) SimHandler.LegendreIntegrator(&E_ref, &V_r, &V_l);
...
void SimManager::Gather()
{
    ...
    ArrayXd  V_r_    = V_r    / mpi_size_;
    ArrayXd  V_l_    = V_l    / mpi_size_;
    ...
    MPI_Reduce(V_r_   .data(), V_r   .data(), V_r   .size(), MPI_DOUBLE, MPI_SUM, MPI_MASTER, MPI_COMM_WORLD);
    MPI_Reduce(V_l_   .data(), V_l   .data(), V_l   .size(), MPI_DOUBLE, MPI_SUM, MPI_MASTER, MPI_COMM_WORLD);
